...
#define UI_RADAR_byte       16    // 1+15#define UI_Data_SENTRY_CMD  0x120  // 哨兵自主决策指令
#define UI_Sentry_byte      19    // 4+15
#define REFEREE_ServerID    0x8080  // 裁判系统服务器（用于哨兵和雷达自主决策指令）
#define UI_Data_SENTRY_CMD  0x0120
#define NULL 0
...
//    uint8_t attitude_mode:2;//哨兵修改当前姿态指令,1为进攻姿态，2为防御姿态，3为移动姿态
//    uint8_t sentry_judge_big_buff:1; //哨兵机器人是否确认使能量机关进入正在激活状态， 1为确认。默认0

//    uint16_t reserve:8;    // 保留
//} Sentry_cmd_t;      // 哨兵自主决策指令
typedef __packed struct
{
    uint16_t if_revive:1;    /* 哨兵机器人是否确认复活 */
                            // 0 表示哨兵机器人确认不复活，即使此时哨兵的复活读条已经完成
                            // 1 表示哨兵机器人确认复活，若复活读条完成将立即复活
    uint16_t if_immediately_revive:1;    /* 哨兵机器人是否确认兑换立即复活 */
                                        // 0 表示哨兵机器人确认不兑换立即复活
                                        // 1 表示哨兵机器人确认兑换立即复活，若此时哨兵机器人
                                        // 符合兑换立即复活的规则要求，则会立即消耗金币兑换立
                                        // 即复活
    uint16_t exchange_projectile_num:11;    /* 哨兵将要兑换的发弹量值，开局为 0，修改此值后，哨兵在补血点即可兑换允许发弹量 */
                                            // 此值的变化需要单调递增，否则视为不合法
    uint16_t remote_exchange_projectile_count:4;    /* 哨兵远程兑换发弹量的请求次数，开局为 0，修改此值即可请求远程兑换发弹量 */
                                                   // 此值的变化需要单调递增且每次仅能增加 1，否则视为不合法
    uint16_t remote_exchange_HP_count:4;    /* 哨兵远程兑换血量的请求次数，开局为 0，修改此值即可请求远程兑换血量 */
                                           // 此值的变化需要单调递增且每次仅能增加 1，否则视为不合法
    /* 在哨兵发送该子命令时，服务器将按照从相对低位到相对高位的原则依次处理这些指令，直至全部成功或不能处理为止 */
    uint16_t attitude_mode:2;//哨兵修改当前姿态指令,1为进攻姿态，2为防御姿态，3为移动姿态，切换有5s冷却
    uint16_t sentry_judge_big_buff:1; //哨兵机器人是否确认使能量机关进入正在激活状态， 1为确认。默认0
