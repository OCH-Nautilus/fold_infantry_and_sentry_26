...
void usart1_init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)
{
    ...
    /*
     * ?? 原 USART6_RX 在 DMA Stream 1 → TCIF1
     *    查 CubeMX 确认 USART1_RX 所在 Stream 编号 N，改为 TCIF{N}
     */
    __HAL_DMA_CLEAR_FLAG(&hdma_usart1_rx, DMA_LISR_TCIF1);   // ← 改成 USART1_RX 对应 Stream
...
void usart1_tx_dma_enable(uint8_t *data, uint16_t len)
{
    ...
    __HAL_DMA_CLEAR_FLAG(&hdma_usart1_tx, DMA_HISR_TCIF6);  //  改成 USART1_TX 对应 Stream
